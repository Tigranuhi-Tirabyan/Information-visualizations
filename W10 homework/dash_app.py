...
df_survival = merged_df.groupby(['Timepoint','Drug Regimen'],as_index=False).count()
df_metastatic = merged_df.groupby(['Timepoint', 'Drug Regimen'], as_index = False).mean()
...
def update_survival_linechart(survival_list):
    traces = []
    if survival_list:
